      ...
      chrome.runtime.sendMessage(
        {
          ...
          payload: { company: j?.company ?? null, role: j?.title ?? null, pageUrl },
        },
        () => {
  ...
  return (
    ...
    >
      ...
      <span
        style={{
          fontSize: '13px',
          color: value ? 'var(--vt-text)' : 'var(--vt-text-disabled)',
...
export default function AutofillPanel() {
      ...
      (response: unknown) => {
        ...
        if (skipped.length > 0) {
          ...
          chrome.runtime.sendMessage(
            ...
            (aiFillResp: unknown) => {
              void chrome.runtime.lastError;
              const r = aiFillResp as
                | { ok?: boolean; aiFilled?: number; remaining?: SkippedField[] }
                | null
                | undefined;
              setState({
                status: 'complete',
                filled: fieldsFilled + (r?.aiFilled ?? 0),
                aiFilled: r?.aiFilled ?? 0,
                skipped: r?.remaining ?? skipped,
              });
  ...
  function handleSubmitRequest(): void {
    ...
    setState({
      status: 'confirming-submit',
      filled: state.filled,
      aiFilled: state.aiFilled,
  ...
  function handleSubmitConfirm(): void {
    ...
    chrome.runtime.sendMessage({ type: 'REQUEST_SUBMIT' }, (response: unknown) => {
      ...
      if (resp?.ok) {
        ...
        setState({
          status: 'error',
          message: 'Could not find the submit button. Please submit the form manually.',
        });
      }
    });
  }
  ...
  return (
    ...
    >
      ...
      {state.status === 'ai-filling' && (
        <p style={{ fontSize: '12px', color: 'var(--vt-text-muted)', margin: 0 }}>
          ✨ AI filling remaining fields&hellip;
        </p>
      )}
      ...
      {state.status === 'submitted' && (
        <div>
          ...
          <button onClick={() => setState({ status: 'idle' })} style={btnStyle('secondary')}>
            Done
          </button>
        </div>
      )}
              ...
              <p style={{ fontSize: '12px', color: 'var(--vt-text-muted)', margin: '0 0 8px' }}>
                ...
                {state.form.unknownFieldCount > 0 && (
                  <span style={{ color: 'var(--vt-warning)', marginLeft: 6 }}>
                    &middot; &#9888; {state.form.unknownFieldCount} need
                    {state.form.unknownFieldCount === 1 ? 's' : ''} review
                  </span>
                )}
              </p>
              <div style={{ marginBottom: '12px' }}>
                {state.form.fillableFields.map((f) => (
                  <FieldRow key={f.profileKey} field={f} profile={state.profile} />
                ))}
                {manualFields.map((f) => (
                  <ManualFieldRow key={f.label} field={f} />
                ))}
              </div>
            ...
            <div>
              ...
              {state.skipped.length > 0 && (
                ...
                >
                  ...
                  >
                    &#9888; {state.skipped.length} field{state.skipped.length !== 1 ? 's' : ''} need
                    {state.skipped.length === 1 ? 's' : ''} review:
                  </p>
                  {state.skipped.map((s) => (
                    <p
                      key={s.label}
                      style={{ fontSize: '11px', color: 'var(--vt-text-muted)', margin: '2px 0 0' }}
                    >
                      &middot; {s.label}
                    </p>
                  ))}
              ...
              {isConfirming ? (
                ...
                >
                  ...
                  >
                    This will click the submit button on the form. Make sure you&apos;ve reviewed
                    all answers first.
                  </p>
                ...
                <>
                  ...
                  <button
                    ...
                    style={{ ...btnStyle('primary'), marginBottom: '8px' }}
                  >
                    Submit Application
                  </button>
                </>
