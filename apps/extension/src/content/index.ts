...
function runDetection(): void {
  ...
  if (fields.length > 0) {
    ...
    const manualFields = fields
      ...
      .map((f) => ({ label: f.label ?? 'File upload', required: f.required }));
    ...
    console.log(
      `[DVantage Content] FORM_DETECTED — fillable:${fillableFields.length} manual:${manualFields.length} unknown:${unknownFieldCount}`,
    );
  } else {
...
chrome.runtime.onMessage.addListener(
  (
    ...
    sendResponse: (
      response: AutofillExecutionResponse | AiFillExecutionResponse | SubmitExecutionResponse,
    ) => void,
  ): boolean | undefined => {
    ...
    if (type === 'EXECUTE_AUTOFILL') {
      ...
      if (
        ...
        (sendResponse as (r: AutofillExecutionResponse) => void)({
          ok: false,
      ...
      try {
        ...
        console.log(
          `[DVantage Content] EXECUTE_AUTOFILL — filled:${result.filled} skipped:${result.skipped.length}`,
        );
      } catch (err) {
        (sendResponse as (r: AutofillExecutionResponse) => void)({
          ok: false,
          error: err instanceof Error ? err.message : 'fill_error',
        });
    ...
    if (type === 'EXECUTE_AI_FILL') {
      ...
      if (
        ...
        (sendResponse as (r: AiFillExecutionResponse) => void)({
          ok: false,
          error: 'invalid_payload',
        });
        return true;
      }
      const answers = (payload as Record<string, unknown>)['answers'] as Array<{
        label: string;
        value: string;
        selector: string;
        fieldType: string;
      ...
      for (const answer of answers) {
        ...
        if (answer.selector)
          el = document.querySelector<HTMLInputElement | HTMLTextAreaElement>(answer.selector);
    ...
    if (type === 'EXECUTE_SUBMIT') {
      ...
      if (!submitEl) {
        (sendResponse as (r: SubmitExecutionResponse) => void)({
          ok: false,
      ...
      console.log(
        '[DVantage Content] EXECUTE_SUBMIT — clicked:',
        submitEl.textContent?.trim().slice(0, 30),
      );
      return true;
    }
...
const adapterName = window.location.hostname.endsWith('linkedin.com')
  ? 'linkedin'
  : window.location.hostname.endsWith('indeed.com')
    ? 'indeed'
    : window.location.hostname === 'boards.greenhouse.io' ||
        window.location.hostname === 'job-boards.greenhouse.io'
      ? 'greenhouse'
      : window.location.hostname === 'jobs.lever.co'
        ? 'lever'
        : window.location.hostname === 'jobs.ashbyhq.com'
          ? 'ashby'
          : window.location.hostname.endsWith('.myworkdayjobs.com')
            ? 'workday'
            : 'generic';

console.log(
  '[DVantage Content] Dispatcher ready | adapter:',
  adapterName,
  '| host:',
  window.location.hostname,
);
