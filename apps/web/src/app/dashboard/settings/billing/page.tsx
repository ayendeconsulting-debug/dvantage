...
function UsageMeter({ label, used, limit }: { label: string; used: number; limit: number | null }) {
  ...
  const barColor = isFull
    ? 'var(--vt-status-danger)'
    : isWarn
      ? 'var(--vt-status-warning)'
      : 'var(--vt-brand-500)';
...
function BillingContent() {
  ...
  async function handleUpgrade() {
    // The price is resolved server-side from the interval. The client no
    // longer knows or sends a Stripe price ID — see lib/api/subscription.ts.
    setActionLoading(true);
    setActionError(null);
    try {
      const { checkoutUrl } = await createCheckoutSession({ interval: 'monthly' });
      ...
      {success && (
        <div
          style={{
            ...
            border: '1px solid var(--vt-status-success)',
            borderRadius: '8px',
            marginBottom: '20px',
          }}
        >
          <CheckCircle2
            size={16}
            strokeWidth={1.5}
            style={{ color: 'var(--vt-status-success)', flexShrink: 0 }}
          />
          <span
            style={{
              fontFamily: 'var(--vt-font-body)',
      ...
      {canceled && (
        <div
          style={{
            ...
            border: '1px solid var(--vt-status-warning)',
            borderRadius: '8px',
            marginBottom: '20px',
          }}
        >
          <AlertCircle
            size={16}
            strokeWidth={1.5}
            style={{ color: 'var(--vt-status-warning)', flexShrink: 0 }}
          />
          <span
            style={{
              fontFamily: 'var(--vt-font-body)',
              fontSize: '13.5px',
              color: 'var(--vt-status-warning)',
            }}
          >
          ...
          >
            ...
            >
              <div>
                ...
                >
                  {isPremium && (
                    <Crown size={16} strokeWidth={1.5} style={{ color: 'var(--vt-brand-400)' }} />
                  )}
                  <span
                    style={{
                      fontFamily: 'var(--vt-font-display)',
                  ...
                  <span
                    style={{
                      ...
                      padding: '2px 8px',
                      borderRadius: '999px',
                      backgroundColor: isPremium ? '#1e3a5f' : 'var(--vt-surface-border)',
                      color: isPremium ? 'var(--vt-brand-400)' : 'var(--vt-text-muted)',
                    }}
                  >
