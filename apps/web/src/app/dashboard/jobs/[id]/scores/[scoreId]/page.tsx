            ...
            >
              <div
                style={{
                  height: '100%',
                  width: `${before}%`,
                  backgroundColor: 'var(--vt-text-disabled)',
                  borderRadius: '3px',
                  opacity: 0.45,
                }}
              />
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Keyword gaps{' '}
          <span style={{ color: 'var(--vt-status-danger)', fontFamily: 'var(--vt-font-mono)' }}>
      ...
      >
        ...
        >
          Matched keywords{' '}
          <span style={{ color: 'var(--vt-status-success)', fontFamily: 'var(--vt-font-mono)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            ...
            <div
              style={{
                ...
                backgroundColor: 'var(--vt-surface-overlay)',
                borderRadius: '6px',
                ...(isChanged('summary') ? highlightStyle : {}),
              }}
            >
              <p
                style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
          ...
          <div
            style={{
              ...
              backgroundColor: 'var(--vt-surface-overlay)',
              borderRadius: '6px',
              ...(isChanged('skills') ? highlightStyle : {}),
            }}
          >
...
export default function ScoreDetailPage() {
    ...
    try {
      ...
      if (data.scoringStatus === 'pending' || data.scoringStatus === 'scoring') {
        ...
        if (data.optimizationStatus === 'complete' || data.optimizationStatus === 'failed') {
          ...
          data.optimizationStatus === 'optimizing'
        ) {
          setOptLoading(true);
          void pollOptimization();
        }
  ...
  async function handleExportPdf() {
    ...
    setExportError(null);
    try {
      await triggerOptimizedExport(
        jobId,
        scoreId,
        'pdf',
        optimization.optimizedData.contact?.name ?? 'resume',
      );
    } catch (err) {
  ...
  const OptimizeControls = (
    ...
    >
      ...
      {(score?.optimizationStatus === 'pending' ||
        score?.optimizationStatus === 'optimizing' ||
        optLoading) && (
        <div
          style={{
            display: 'inline-flex',
    ...
    <div>
      ...
      {(loading || score?.scoringStatus === 'pending' || score?.scoringStatus === 'scoring') && (
        ...
        >
          <Loader
            size={28}
            strokeWidth={1.5}
            style={{ color: 'var(--vt-brand-400)', animation: 'spin 1s linear infinite' }}
          />
          <p
            style={{
              fontFamily: 'var(--vt-font-display)',
      ...
      {score?.scoringStatus === 'failed' && (
        ...
        >
          ...
          Scoring failed: {score.scoreError ?? 'Unknown error.'}
        </div>
      )}
                      ...
                      {optimizedSections && (
                        ...
                        >
                          <TrendingUp
                            size={14}
                            strokeWidth={1.5}
                            style={{ color: 'var(--vt-status-success)' }}
                          />
                          <span
                            style={{
                              fontFamily: 'var(--vt-font-body)',
