    ...
    try {
      ...
      if (TERMINAL.includes(res.parseStatus as (typeof TERMINAL)[number])) {
        ...
        if (res.parseStatus === 'complete') {
          const opts = await listOptimizationsForResume(id).catch(() => ({
            data: [] as ResumeOptimizationItem[],
          }));
    ...
    <div style={{ maxWidth: '720px' }}>
      ...
      >
        <div>
          ...
          >
            v{data.versionNumber} · {(data.fileSize / 1024).toFixed(0)} KB ·{' '}
        ...
        {isComplete && (
          ...
          >
            ...
            <button
              ...
              title={
                selectedExport === 'original'
                  ? 'Download as formatted PDF'
                  : 'Download optimized version as PDF'
              }
            >
              {exportingPdf ? (
                <Loader
            ...
            <button
              ...
              title={
                selectedExport === 'original'
                  ...
                  : 'Download optimized version as Word document'
              }
            >
              {exportingDocx ? (
                <Loader
      ...
      {isParsing && (
        ...
        >
          <Loader size={15} strokeWidth={1.5} style={{ animation: 'spin 1s linear infinite' }} />
          Analysing your resume — this usually takes 20–40 seconds.
        </div>
      )}
      ...
      {isFailed && (
        ...
        >
          <AlertCircle size={15} strokeWidth={1.5} />
          Analysis failed: {data.parseError ?? 'Unknown error.'}
        </div>
      )}
            ...
            {sd.summary && (
              <p
                style={{
                  ...
                  lineHeight: 1.7,
                  margin: '16px 0 0',
                  borderTop: '1px solid var(--vt-surface-border)',
                  paddingTop: '16px',
                }}
              >
                {sd.summary}
                ...
                {sd.skills.map((s, i) => (
                  <span
                    ...
                    style={{
                      ...
                      fontSize: '12px',
                      color: 'var(--vt-text-body)',
                      backgroundColor: SKILL_BG[s.category] ?? 'var(--vt-surface-border)',
                      border: '1px solid var(--vt-surface-border)',
                      padding: '3px 10px',
                      borderRadius: '999px',
                    }}
