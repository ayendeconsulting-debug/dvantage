...
export default function ForgotPasswordPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!resp.ok) { setError(data.message ?? 'Could not send reset email.'); } else { setSent(true); }
    setLoading(false);
  }
  ...
  return (
    <AuthCard title="Reset password" subtitle="We'll send you a reset link.">
      ...
      <AuthLink>Remember it? <Link href="/auth/sign-in" style={{ color: 'var(--vt-brand-400)', textDecoration: 'none' }}>Sign in</Link></AuthLink>
    </AuthCard>
  );
}